...
for i in range(len(mmf)):
    ...
    holding = round(float(price) * mmf[i]['holding_celi']+ float(price) * mmf[i]['holding_reer'] + float(price) * mmf[i]['holding_cri'])

    celi = celi + round(float(price) * mmf[i]['holding_celi'])
    reer = reer + round(float(price) * mmf[i]['holding_reer'])
    cri = cri + round(float(price) * mmf[i]['holding_cri'])
    data['mmf'].append({'ticker': mmf[i]['ticker'],
                        'price': price,
                        'holding': holding})

total = round(celi + reer + cri)
data['holding'].append({"holding_celi": celi,
             "holding_reer": reer,
             "holding_cri": cri,
             "holding_total": total
             })
