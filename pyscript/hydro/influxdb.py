...
from consts import DAILY_MAP, CURRENT_MAP, ANNUAL_MAP, HOURLY_MAP
try:
...
class InfluxDB:
    ...
    def __init__(self, params):
        ...
        if self.bucket is None:
            raise ValueError(
                "Missing bucket name, please provide one in db_params")
        self.connect_to_db()
    ...
    def write_data_to_db(self, customer, show_hourly=True):
        print(customer.current_daily_data)
        _data = []
        ...
        for k, v in CURRENT_MAP.items():
            _id = "{}|_current_{}".format(id_prefix, k)
            _point = (
                Point(_id)
        ...
        for k, v in DAILY_MAP.items():
            _id = "{}|_daily_{}".format(id_prefix, k)
            _point = (
                Point(_id)
                .field("value", customer.current_daily_data[yesterday_date][k])
                .tag("category", "hourly")
                .tag("unit", v['unit'])
                .tag('name', v['raw_name'])
                .tag("account_id", account_id)
        ...
        if show_hourly:
            ...
            for hour, _data in customer.hourly_data[yesterday_date]["hours"].items():
                for k, v in DAILY_MAP.items():
                    _id = "{}|_hourly_{}".format(id_prefix, k)
                    _point = (
                        Point(_id)
                        .field("value", _data[k])
                        .tag("category", "hourly")
