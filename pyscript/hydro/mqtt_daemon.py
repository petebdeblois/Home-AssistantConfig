...
from client import HydroQuebecClient
from consts import DAILY_MAP, CURRENT_MAP, HQ_TIMEZONE
...
class MqttHydroQuebec(mqtt_hass_base.MqttDevice):
    ...
    def _publish_sensor(self, sensor_type, contract_id,
        ...
        sensor_config.update({
            ...
            "name": "test_hydroquebec_{}_{}".format(contract_id, sensor_type),
    ...
    async def _main_loop(self):
        ...
        for account in self.config['accounts']:
            ...
            for contract_data in account['contracts']:
                ...
                for data_name, data in HOURLY_MAP.items():
                    # Publish sensor
                    sensor_topic = self._publish_sensor('hourly_' + data_name,
                                                        customer.contract_id,
                                                        unit=data['unit'],
                                                        icon=data['icon'],
                                                        device_class=data['device_class'])
                    ...
                    self.mqtt_client.publish(
                            topic=sensor_topic,
                            payload=customer.hourly_data[yesterday_str][data_name])
